...
class OptimizerReplacement(object):
    ...
    def optimizer_placement_info(self, optimizer_init, placement):
        # 某个放置方案下的结果信息
        self.get_best_worst_node(optimizer_init)
        placement_info = self.get_placement_min_info(placement, optimizer_init)
        placement_info = self.update_opt_result(placement_info, optimizer_init)
    ...
    def optimizer_replacement(self, optimizer_init):
        ...
        opt_result = self.update_opt_result(opt_result, optimizer_init)
    ...
    def update_opt_result(self, opt_result, optimizer_init):
    ...
    def replacement_neighbor_placement(self, placement_info, optimizer_init):
        # 多次替换，直到下一次替换和当前值相同，说明找到（全局/局部）最优
        replacement_res_list = []
        while True:
...
if __name__ == '__main__':
    ...
    jcsproxy_request_features = {
        'aliyun-beijing': 3,      # 注意不能全是0，最低上传的porxy，下载次数为0.1
        'aliyun-shanghai': 0,
        'aliyun-shenzhen': 3,
    }
    file_size = 1024
    ...
    print 'replacement_res_list', replacement_res_list
    print 'replacement_res_list len', len(replacement_res_list)
    for i in range(len(replacement_res_list)):
        print replacement_res_list[i]['placement']
